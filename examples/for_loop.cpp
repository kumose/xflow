...
int main(int argc, const char *argv[]) {
    ...
    std::cout << "in-graph control flow time    : " << std::chrono::duration_cast<
                std::chrono::microseconds>(tend - tbeg).count()
            << " us\n";

    // expressing the loop without using cointrol taskflow graph
    xf::Taskflow taskflow_x, taskflow_y, taskflow_z;
    taskflow_x.emplace([]() {
    });
    ...
    std::cout << "out-of-graph control flow time: " << std::chrono::duration_cast<
                std::chrono::microseconds>(tend - tbeg).count()
            << " us\n";
