/**
 ...
 we disable this behavior by calling `xf::Subflow::retain(true)`.
...
int main() {
    ...
    taskflow.for_each_task([](xf::Task task) {
        std::cout << "task " << task.name() << " [type=" << xf::to_string(task.type()) << "]\n";
        // traverse it's successor
        task.for_each_successor([](xf::Task successor) {
            std::cout << "  -> successor   task " << successor.name() << '\n';
        });
        // traverse it's predecessor
        task.for_each_predecessor([](xf::Task predecessor) {
        ...
        task.for_each_subflow_task([](xf::Task stask) {
            ...
            // traverse it's successor
            stask.for_each_successor([](xf::Task successor) {
                std::cout << "    -> successor   task " << successor.name() << '\n';
            });
            // traverse it's predecessor
            stask.for_each_predecessor([](xf::Task predecessor) {
                std::cout << "    <- predecessor task " << predecessor.name() << '\n';
            });
        });
    });
