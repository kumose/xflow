...
class CustomWorkerBehavior : public xf::WorkerInterface {
  ...
  void scheduler_prologue(xf::Worker& w) override {
  ...
  void scheduler_epilogue(xf::Worker& w, std::exception_ptr) override {
...
int main() {
  ...
  xf::Executor executor(4, xf::make_worker_interface<CustomWorkerBehavior>());
  return 0;
}
