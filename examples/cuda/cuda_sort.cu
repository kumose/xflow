...
int main(int argc, char* argv[]) {
  ...
  // gpu data
  auto d_keys = xf::cuda_malloc_shared<int>(N);
  ...
  auto p = xf::cudaDefaultExecutionPolicy{};
  
  auto beg = std::chrono::steady_clock::now();
  xf::cudaStream s;
  auto bufsz = xf::cuda_sort_buffer_size<decltype(p), int>(N);
  xf::cudaDeviceVector<std::byte> buf(bufsz);
  xf::cuda_sort(p, d_keys, d_keys+N, xf::cuda_less<int>{}, buf.data());
  s.synchronize();
