...
void joined_subflow_1(unsigned W) {
  ...
  taskflow.emplace([&] (xf::Subflow& sf0) {
    for (int i = 0; i < 100; ++i) {
      sf0.emplace([&] (xf::Subflow& sf1) {
...
void joined_subflow_3(unsigned N) {
  ...
  // implicit join
  taskflow.emplace([&](xf::Subflow& sf) {
    xf::Task W = sf.emplace([&](){ ++num_tasks; });
    xf::Task X = sf.emplace([&](){ ++num_tasks; throw std::runtime_error("x"); });
    xf::Task Y = sf.emplace([&](){ ++num_tasks; });
    xf::Task Z = sf.emplace([&](){ ++num_tasks; });
    W.precede(X);
    X.precede(Y);
    Y.precede(Z);
  });
  ...
  taskflow.emplace([&](xf::Subflow& sf) {
    xf::Task W = sf.emplace([&](){ ++num_tasks; });
    xf::Task X = sf.emplace([&](){ ++num_tasks; throw std::runtime_error("y"); });
    xf::Task Y = sf.emplace([&](){ ++num_tasks; });
    xf::Task Z = sf.emplace([&](){ ++num_tasks; });
    W.precede(X);
    X.precede(Y);
...
void nested_subflow(unsigned N) {
  ...
  // level 1
  taskflow.emplace([&](xf::Subflow& sf1) {
    xf::Task V1 = sf1.emplace([&num_tasks](){ ++num_tasks; }).name("V1");
    xf::Task W1 = sf1.emplace([&num_tasks](){ ++num_tasks; }).name("W1");
    
    // level 2
    xf::Task X1 = sf1.emplace([&num_tasks](xf::Subflow& sf2){
      ++num_tasks; 

      xf::Task V2 = sf2.emplace([&num_tasks](){ ++num_tasks; }).name("V2");
      xf::Task W2 = sf2.emplace([&num_tasks](){ ++num_tasks; }).name("W2");
      
      // level 3
      xf::Task X2 = sf2.emplace([&num_tasks](xf::Subflow& sf3) {
        ++num_tasks;

        xf::Task V3 = sf3.emplace([&num_tasks](){ ++num_tasks; }).name("V3");
        xf::Task W3 = sf3.emplace([&num_tasks](){ ++num_tasks; }).name("W3");

        // level 4
        xf::Task X3 = sf3.emplace([&num_tasks](xf::Subflow& sf4){
          ++num_tasks;

          xf::Task V4 = sf4.emplace([&num_tasks](){ ++num_tasks; }).name("V4");
          xf::Task W4 = sf4.emplace([&num_tasks](){ ++num_tasks; }).name("W4");
          xf::Task X4 = sf4.emplace([&num_tasks](){
            ++num_tasks; 
            throw std::runtime_error("x");
          }).name("X4 (throw)");
          xf::Task Y4 = sf4.emplace([&num_tasks](){ ++num_tasks; }).name("Y4");
          xf::Task Z4 = sf4.emplace([&num_tasks](){ ++num_tasks; }).name("Z4");
        ...
        xf::Task Y3 = sf3.emplace([&num_tasks](){ ++num_tasks; }).name("Y3");
        xf::Task Z3 = sf3.emplace([&num_tasks](){ ++num_tasks; }).name("Z3");
      ...
      xf::Task Y2 = sf2.emplace([&num_tasks](){ ++num_tasks; }).name("Y2");
      xf::Task Z2 = sf2.emplace([&num_tasks](){ ++num_tasks; }).name("Z2");
    ...
    xf::Task Y1 = sf1.emplace([&num_tasks](){ ++num_tasks; }).name("Y1");
    xf::Task Z1 = sf1.emplace([&num_tasks](){ ++num_tasks; }).name("Z1");
  ...
  num_tasks = 0;
  executor.silent_async([&](xf::Runtime&){
    //REQUIRE_THROWS_WITH_AS(rt.corun(taskflow), "x", std::runtime_error);
    REQUIRE_THROWS_WITH_AS(executor.corun(taskflow), "x", std::runtime_error);
  });
  executor.wait_for_all(); 
...
void executor_corun_1(unsigned W) {
  ...
  xf::Executor executor(W);
  xf::Taskflow taskflow1;
  xf::Taskflow taskflow2;
  ...
  for(size_t i=0; i<100; i++) {
    taskflow1.emplace([](xf::Subflow& sf){
...
void runtime_corun_1(unsigned W) {
  ...
  auto task = taskflow2.emplace([&](xf::Runtime&){
    REQUIRE_THROWS_WITH_AS(executor.corun(taskflow1), "x", std::runtime_error);
  });
  executor.run(taskflow2).get();
...
void async_with_runtime(unsigned W) {
  ...
  for(size_t i=0; i<1024; i++) {
    futures.emplace_back(executor.async([](xf::Runtime&){
      throw std::runtime_error("x");
    }));
  }
...
void dependent_async_with_runtime(unsigned W) {
  ...
  for(size_t i=0; i<1024; i++) {
    auto [t, f] = executor.dependent_async([](xf::Runtime&){
      throw std::runtime_error("x");
    });
  }
  ...
  // silently caught by the task
  executor.silent_dependent_async([](xf::Runtime&){
    throw std::runtime_error("x");
  });
...
void runtime_async_task(unsigned W) {
  ...
  // runtime async
  auto A = taskflow.emplace([](xf::Runtime& rt){
  ...
  flag = 0;
  taskflow.clear();
  A = taskflow.emplace([&](xf::Runtime& rt){
...
void semaphore1(unsigned W) {
  ...
  xf::Task B = taskflow.emplace([](){ throw std::runtime_error("exception"); });
  xf::Task C = taskflow.emplace([](){});
  xf::Task D = taskflow.emplace([](){});
