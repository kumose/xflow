...
void dynamic_traversal(unsigned W) {
  ...
  std::function<void(Node*, xf::Subflow&)> traverse;

  traverse = [&] (Node* n, xf::Subflow& subflow) {
    REQUIRE(!n->visited);
    n->visited = true;
    ...
    for(size_t i=0; i<S; i++) {
      if(n->successors[i]->dependents.fetch_sub(1) == 1) {
        ...
        subflow.emplace([s=n->successors[i], &traverse](xf::Subflow &subflow2){
          traverse(s, subflow2);
        });
      }
    }
  ...
  for(size_t i=0; i<src.size(); i++) {
    tf.emplace([s=src[i], &traverse](xf::Subflow& subflow){
      traverse(s, subflow);
    });
  }
    ...
    threads.emplace_back([&](){
      ...
      traverse = [&] (Node* n, xf::Subflow& subflow) {
        ...
        for(size_t i=0; i<S; i++) {
          if(n->successors[i]->dependents.fetch_sub(1) == 1) {
            n->successors[i]->level = ++level;
            subflow.emplace([s=n->successors[i], &traverse](xf::Subflow &subflow){
              traverse(s, subflow);
            });
          }
        }
