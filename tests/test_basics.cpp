...
TEST_CASE("Type" * doctest::timeout(300)) {
  ...
  xf::Taskflow taskflow, taskflow2;

  auto t1 = taskflow.emplace([](){});
  auto t2 = taskflow.emplace([](){ return 1; });
  auto t3 = taskflow.emplace([](xf::Subflow&){ });
  auto t4 = taskflow.composed_of(taskflow2);
  auto t5 = taskflow.emplace([](){ return xf::SmallVector{1, 2}; });
  auto t6 = taskflow.emplace([](xf::Runtime&){});
  auto t7 = taskflow.emplace([](xf::NonpreemptiveRuntime&){});

  REQUIRE(t1.type() == xf::TaskType::STATIC);
  REQUIRE(t2.type() == xf::TaskType::CONDITION);
  REQUIRE(t3.type() == xf::TaskType::SUBFLOW);
  REQUIRE(t4.type() == xf::TaskType::MODULE);
  REQUIRE(t5.type() == xf::TaskType::CONDITION);
  REQUIRE(t6.type() == xf::TaskType::RUNTIME);
  REQUIRE(t7.type() == xf::TaskType::RUNTIME);

  // static assert
  auto task1 = [](){};
  auto task2 = [](){ return 1; };
  auto task3 = [](xf::Subflow&) {};
  auto task4 = [](xf::Subflow&) { return 1; };
  auto task5 = [](xf::Runtime&) {};
  auto task6 = [](xf::Runtime&) { return 1; };
  auto task7 = [](xf::NonpreemptiveRuntime&) {};
  auto task8 = [](xf::NonpreemptiveRuntime&) { return 1; };

  static_assert(xf::is_static_task_v<decltype(task1)> == true, "");
  static_assert(xf::is_static_task_v<decltype(task2)> == false, "");
  static_assert(xf::is_condition_task_v<decltype(task2)> == true, "");
  static_assert(xf::is_subflow_task_v<decltype(task3)> == true, "");
  static_assert(xf::is_subflow_task_v<decltype(task4)> == false, "");
  static_assert(xf::is_runtime_task_v<decltype(task5)> == true, "");
  static_assert(xf::is_runtime_task_v<decltype(task6)> == false, "");
  static_assert(xf::is_runtime_task_v<decltype(task7)> == true, "");
  static_assert(xf::is_runtime_task_v<decltype(task8)> == false, "");
}
...
TEST_CASE("Builder" * doctest::timeout(300)) {
  ...
  std::vector<xf::Task> silent_tasks;
...
TEST_CASE("Iterators" * doctest::timeout(300)) {
  ...
  SUBCASE("Order") {
    ...
    A.for_each_successor([&, i=0] (xf::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == B);
        break;
    ...
    E.for_each_predecessor([&, i=0](xf::Task s) mutable {
      switch(i++) {
        case 0:
          REQUIRE(s == A);
        break;
  ...
  SUBCASE("Generic") {
    ...
    taskflow.for_each_task([&tasks](xf::Task s){
      tasks.push_back(s);
    });
    ...
    A.for_each_successor([B](xf::Task s){ REQUIRE(s==B); });
    B.for_each_predecessor([A](xf::Task s){ REQUIRE(s==A); });
    ...
    A.for_each_successor([&](xf::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
    ...
    counter = a = b = c = d = e = 0;
    B.for_each_predecessor([&](xf::Task s) {
      counter++;
      if(s == A) ++a;
      if(s == B) ++b;
      if(s == C) ++c;
...
TEST_CASE("RunAndWait.Simple") {
  ...
  xf::Taskflow others;
  xf::Task A = others.emplace([&](){ counter++; });
  xf::Task B = others.emplace([&](){ counter++; });
  A.precede(B);
...
void worker_id(unsigned w) {
  ...
  for(int i=0; i<1000; i++) {
    ...
    auto B = taskflow.emplace([&](xf::Subflow& sf){
      auto id = executor.this_worker_id();
      REQUIRE(id>=0);
      REQUIRE(id< w);
      sf.emplace([&](){
...
void parallel_runs(unsigned w) {
  ...
  auto make_taskflow = [&] (xf::Taskflow& tf) {
  ...
  SUBCASE("RunAndWaitForAll") {
    xf::Executor executor(w);
    counter = 0;
    std::vector<std::unique_ptr<xf::Taskflow>> taskflows(32);
    std::atomic<size_t> barrier(0);
    for(int t=0; t<32; t++) {
      threads.emplace_back([&, t=t] () {
        taskflows[t] = std::make_unique<xf::Taskflow>();
...
void observer(unsigned w) {
  ...
  auto observer = executor.make_observer<xf::ChromeObserver>();

  xf::Taskflow taskflowA;
  std::vector<xf::Task> tasks;
  // Static tasking
  for(auto i=0; i < 64; i ++) {
