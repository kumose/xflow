...
TEST_CASE("Cond.Types" * doctest::timeout(300)) {
  ...
  static_assert(xf::is_condition_task_v<decltype(explicit_c)>);
  static_assert(xf::is_condition_task_v<decltype(implicit_c)>);

  REQUIRE(explicit_task.type() == xf::TaskType::CONDITION);
  REQUIRE(implicit_task.type() == xf::TaskType::CONDITION);
}
  ...
  if(depth < max_depth) {
    for(int i=0; i<2; i++) {
      ...
      auto B = subflow.emplace(
        [&, max_depth, depth=depth+1](xf::Subflow& sf2){
...
TEST_CASE("DynamicBTreeCondition" * doctest::timeout(300)) {
  for(unsigned w=1; w<=8; ++w) {
    ...
    xf::Taskflow flow;
    flow.emplace([&](xf::Subflow& subflow) {
      counter++;
...
void nested_cond(unsigned w) {
  ...
  xf::Taskflow flow;
  auto S = flow.emplace([](){});
  auto A = flow.emplace([&] (xf::Subflow& subflow) mutable {
    //           ___________
    //          |           |
    //          v           |
    //   S1 -> A1 -> B1 -> cond
    ...
    auto B1 = subflow.emplace([&](xf::Subflow& sf){
...
void hierarchical_condition(unsigned w) {
  ...
  auto sbf1 = tf3.emplace([&](xf::Subflow& sbf){
...
void condition_subflow(unsigned W) {
  ...
  auto subflow = taskflow.emplace([&](xf::Subflow& sf){
    sf.emplace([&, i](){
      REQUIRE(i<I);
      data[i] = i*(i+1)/2*123;;
...
TEST_CASE("MultiCond.Types" * doctest::timeout(300)) {
  ...
  auto explicit_mc = [](){
    xf::SmallVector<int> v;
    return v;
  };

  auto implicit_mc = []() -> xf::SmallVector<int> {
    return {1, 2, 3, 9};
  };
  ...
  static_assert(xf::is_multi_condition_task_v<decltype(explicit_mc)>);
  static_assert(xf::is_multi_condition_task_v<decltype(implicit_mc)>);
...
void multiple_loops(unsigned W) {
  ...
  auto B = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> xf::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto C = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> xf::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto D = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> xf::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
  ...
  auto E = taskflow.emplace([&, i=bool{true}, c = int(0)]() mutable -> xf::SmallVector<int> {
    if(i) {
      i = false;
      return {0, 1};
    }
...
void binary_tree(unsigned w) {
  ...
  for(int i=1; i<(1<<N); i++) {
    tasks.emplace_back(taskflow.emplace([&counter]() -> xf::SmallVector<int>{
      counter.fetch_add(1, std::memory_order_relaxed);
      return {0, 1};
    }));
  }
...
void multi_cond_multiple_runs(unsigned W) {
  ...
  xf::Task tempTask1 = taskflow.emplace([]() {});
  xf::Task tempTask2 = taskflow.emplace([]() {});
