...
void overlapped_semaphores(size_t W) {
  ...
  xf::Executor executor(W);
  xf::Taskflow taskflow;
  xf::Semaphore semaphore1(1);
  xf::Semaphore semaphore4(4);
...
void conflict_graph(size_t W) {
  ...
  xf::Executor executor(W);
  xf::Taskflow taskflow;
  xf::Semaphore conflict_AB(1);
  xf::Semaphore conflict_AC(1);

  int counter {0};
  std::mutex mutex;

  xf::Task A = taskflow.emplace([&](){ counter++; });
...
void semaphores_in_module(unsigned W) {
  ...
  xf::Taskflow taskflow2;
  xf::Executor executor(W);
  std::vector<xf::Semaphore> semaphores(10);
...
void linear_chain(unsigned W) {
  ...
  std::vector<xf::CachelineAligned<size_t>> counters(L);
  std::vector<xf::Semaphore> semaphores(L);
...
void deadlock(unsigned W) {
  ...
  xf::Executor exec(W);
  xf::Taskflow flow;
  xf::Semaphore semaphore(8);

  for(size_t i = 0; i < 5; ++i) {

    xf::Task commonTask = flow.emplace([i]() {
      std::string str;
    ...
    for(size_t j = 0; j < 8; ++j) {
      xf::Task preTask = flow.emplace([i, j]() {
        std::string str;
      ...
      xf::Task postTask = flow.emplace([i, j]() {
        std::string str;
