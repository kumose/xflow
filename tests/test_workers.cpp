...
class CustomWorkerBehavior : public xf::WorkerInterface {
  ...
  void scheduler_prologue(xf::Worker& wv) override {
    _counter++;
  ...
  void scheduler_epilogue(xf::Worker&, std::exception_ptr) override {
    _counter++;
  }
...
void worker_interface_basics(unsigned W) {
  ...
  {
    xf::Executor executor(W, xf::make_worker_interface<CustomWorkerBehavior>(counter, ids));
  }
