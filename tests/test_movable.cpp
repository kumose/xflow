...
TEST_CASE("moved_run") {
  ...
  // run the moved taskflow
  xf::Executor().run(std::move(taskflow)).wait();
  REQUIRE(counter == 6*N);
  REQUIRE(taskflow.num_tasks() == 0);
  ...
  make_taskflow();
  REQUIRE(taskflow.num_tasks() == N);
  xf::Executor().run_n(std::move(taskflow), 3).wait();
  REQUIRE(counter == 10*N);
  REQUIRE(taskflow.num_tasks() == 0);

  // run the moved empty taskflow with callable
  xf::Executor().run(std::move(taskflow), [&](){
...
TEST_CASE("moved_taskflows") {
  ...
  auto make_taskflow = [&counter](xf::Taskflow& taskflow, int N){
    for(int i=0; i<N; i++) {
  ...
  {
    xf::Taskflow taskflow1;
    xf::Taskflow taskflow2(std::move(taskflow1));

    REQUIRE(taskflow1.num_tasks() == 0);
    REQUIRE(taskflow2.num_tasks() == 0);

    make_taskflow(taskflow1, N);
    xf::Taskflow taskflow3(std::move(taskflow1));
