...
void update_for_each() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto data = xf::cuda_malloc_shared<int>(N);
...
TEST_CASE("cudaFlow.update.for_each" * doctest::timeout(300)) {
  update_for_each<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.for_each" * doctest::timeout(300)) {
  update_for_each<xf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.update.for_each_index" * doctest::timeout(300)) {
  update_for_each_index<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.for_each_index" * doctest::timeout(300)) {
  update_for_each_index<xf::cudaFlowCapturer>();
}
...
void update_reduce() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto data = xf::cuda_malloc_shared<int>(N);
    auto soln = xf::cuda_malloc_shared<int>(1);
...
TEST_CASE("cudaFlow.update.uninitialized_reduce" * doctest::timeout(300)) {
  update_uninitialized_reduce<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.uninitialized_reduce" * doctest::timeout(300)) {
  update_uninitialized_reduce<xf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.update.transform_reduce" * doctest::timeout(300)) {
  update_transform_reduce<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.transform_reduce" * doctest::timeout(300)) {
  update_transform_reduce<xf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.update.transform_uninitialized_reduce" * doctest::timeout(300)) {
  update_transform_uninitialized_reduce<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.transform_uninitialized_reduce" * doctest::timeout(300)) {
  update_transform_uninitialized_reduce<xf::cudaFlowCapturer>();
}
...
void update_transform() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto input1 = xf::cuda_malloc_shared<int>(N);
    auto input2 = xf::cuda_malloc_shared<int>(N);
    auto output = xf::cuda_malloc_shared<int>(N);
...
TEST_CASE("cudaFlow.update.transform" * doctest::timeout(300)) {
  update_transform<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.transform" * doctest::timeout(300)) {
  update_transform<xf::cudaFlowCapturer>();
}
...
TEST_CASE("cudaFlow.update.binary_transform" * doctest::timeout(300)) {
  update_binary_transform<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.binary_transform" * doctest::timeout(300)) {
  update_binary_transform<xf::cudaFlowCapturer>();
}
...
void update_scan() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto input1 = xf::cuda_malloc_shared<int>(N);
    auto input2 = xf::cuda_malloc_shared<int>(N);
    auto output1 = xf::cuda_malloc_shared<int>(N);
    auto output2 = xf::cuda_malloc_shared<int>(N);
...
void update_merge() {
  ...
  for(int N=1; N<=100000; N += (N/10+1)) {
    ...
    auto input1 = xf::cuda_malloc_shared<int>(N);
    auto input2 = xf::cuda_malloc_shared<int>(2*N);
    auto output1 = xf::cuda_malloc_shared<int>(3*N);
    auto output2 = xf::cuda_malloc_shared<int>(3*N);
    ...
    // merge
    auto merge = cf.merge(
      input1, input1+N, input2, input2+2*N, output1, xf::cuda_less<int>()
    );
    cf.offload();
    ...
    // update merge
    cf.merge(merge,
      input1, input1+N, input2, input2+N, output2, xf::cuda_less<int>()
    );
    cf.offload();
...
void update_sort() {
  ...
  for(int N=1; N<=100000; N += (N/100+1)) {
    ...
    cf.sort(sort, input1, input1+N, xf::cuda_greater<int>());
    cf.offload();
...
void update_sort_by_key() {
  ...
  for(int N=1; N<=100000; N += (N/100+1)) {
    ...
    auto input1 = xf::cuda_malloc_shared<int>(N);
    auto input2 = xf::cuda_malloc_shared<int>(N);
    auto index1 = xf::cuda_malloc_shared<int>(N);
    auto index2 = xf::cuda_malloc_shared<int>(N);
...
TEST_CASE("cudaFlow.update.sort_by_key" * doctest::timeout(300)) {
  update_sort_by_key<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.sort_by_key" * doctest::timeout(300)) {
  update_sort_by_key<xf::cudaFlowCapturer>();
}
...
void update_find() {
  ...
  for(unsigned N=1; N<=100000; N += (N/100+1)) {
    ...
    auto input1 = xf::cuda_malloc_shared<int>(N);
    auto input2 = xf::cuda_malloc_shared<int>(N);
    auto index1 = xf::cuda_malloc_shared<unsigned>(1);
    auto index2 = xf::cuda_malloc_shared<unsigned>(1);
...
void update_minmax_element() {
  ...
  for(unsigned N=1; N<=100000; N += (N/100+1)) {
    ...
    // create find
    auto find_min = cf.min_element(input1, input1+N, index1, xf::cuda_less<int>());
    auto find_max = cf.max_element(input2, input2+N, index2, xf::cuda_less<int>());
    cf.offload();
    ...
    // update find
    cf.min_element(find_min, input2, input2+N, index2, xf::cuda_less<int>());
    cf.max_element(find_max, input1, input1+N, index1, xf::cuda_less<int>());
    cf.offload();
...
TEST_CASE("cudaFlow.update.minmax_element" * doctest::timeout(300)) {
  update_minmax_element<xf::cudaFlow>();
}

TEST_CASE("cudaFlowCapturer.update.minmax_element" * doctest::timeout(300)) {
  update_minmax_element<xf::cudaFlowCapturer>();
}
