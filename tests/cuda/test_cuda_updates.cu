...
TEST_CASE("cudaGraph.Update.Kernel") {
  ...
  auto vec = xf::cuda_malloc_shared<int>(N);
...
TEST_CASE("cudaGraph.Update.Memcpy") {
  ...
  auto vec1 = xf::cuda_malloc_shared<int>(N);
  auto vec2 = xf::cuda_malloc_shared<int>(N);
  auto vec3 = xf::cuda_malloc_shared<int>(N);
