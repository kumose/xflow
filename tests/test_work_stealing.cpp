...
void subflow_starvation(size_t W) {
  ...
  taskflow.emplace([&](xf::Subflow& subflow){
...
void skewed_starvation(size_t W) {
  ...
  xf::Task parent = taskflow.emplace([&](){
    set.clear();
...
void nary_starvation(size_t W) {
  ...
  for(size_t w=0; w<W; w++) {
    ...
    for(size_t i=0; i<N; i++) {
      xf::Task task = taskflow.emplace([&, w, p, i](){
...
void wavefront_starvation(size_t W) {
  ...
  std::vector<std::vector<xf::Task>> G;
  G.resize(W);
