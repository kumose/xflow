...
TEST_CASE("SimplePriority.Sequential" * doctest::timeout(300)) {
  ...
  REQUIRE(B.priority() == xf::TaskPriority::HIGH);
  REQUIRE(C.priority() == xf::TaskPriority::HIGH);
  REQUIRE(D.priority() == xf::TaskPriority::HIGH);

  B.priority(xf::TaskPriority::HIGH);
  C.priority(xf::TaskPriority::LOW);
  D.priority(xf::TaskPriority::NORMAL);

  REQUIRE(B.priority() == xf::TaskPriority::HIGH);
  REQUIRE(C.priority() == xf::TaskPriority::LOW);
  REQUIRE(D.priority() == xf::TaskPriority::NORMAL);
...
TEST_CASE("RandomPriority.Sequential" * doctest::timeout(300)) {
  ...
  const auto MAX_P = static_cast<unsigned>(xf::TaskPriority::MAX);
  ...
  for(size_t i=0; i<10000; i++) {
    ...
    taskflow.emplace([p, &counters](){ counters[p]++; })
            .priority(static_cast<xf::TaskPriority>(p))
            .succeed(beg)
            .precede(end);
    priorities[p]++;
  }
