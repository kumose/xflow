...
#include <xflow/cuda/cuda_optimizer.hpp>
...
namespace xf {
    ...
    The usage of xf::cudaFlowCapturer is similar to xf::cudaFlow, except users can
    call the method xf::cudaFlowCapturer::on to capture a sequence of asynchronous
    ...
    @code{.cpp}
    taskflow.emplace([](xf::cudaFlowCapturer& capturer){
    ...
    Similar to xf::cudaFlow, a %cudaFlowCapturer is a task (xf::Task)
    created from xf::Taskflow
    and will be run by @em one worker thread in the executor.
    That is, the callable that describes a %cudaFlowCapturer
    will be executed sequentially.
    Inside a %cudaFlow capturer task, different GPU tasks (xf::cudaTask) may run
    in parallel depending on the selected optimization algorithm.
    By default, we use xf::cudaFlowRoundRobinOptimizer to transform a user-level
    ...
    */
    ...
    public:
        ...
        can be run by the caller thread using xf::cudaFlowCapturer::run.
        */
        ...
        */
        ...
        The method is similar to xf::cudaFlowCapturer::noop but
        ...
        */
        ...
        a user-described %cudaFlow:
          + xf::cudaFlowSequentialOptimizer
          + xf::cudaFlowRoundRobinOptimizer
          + xf::cudaFlowLinearOptimizer

        By default, xf::cudaFlowCapturer uses the round-robin optimization
