...
namespace xf {
    ...
    public:
        ...
        LazyString(const std::string &str) : _str(str.empty() ? nullptr : std::make_unique<std::string>(str)) {
        }

        LazyString(std::string &&str) : _str(str.empty() ? nullptr : std::make_unique<std::string>(std::move(str))) {
        }

        LazyString(const char *str) : _str((!str || str[0] == '\0') ? nullptr : std::make_unique<std::string>(str)) {
        }
