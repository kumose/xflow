...
auto Algorithm::make_module_task(T&& target) {
  return [&target=std::forward<T>(target)](xf::Runtime& rt){
...
@tparam T type of the graph object, which must define the method `xf::Graph& graph()`
...
The module task maker, xf::make_module_task, is basically the same as xf::Taskflow::composed_of
...
xf::Task m1 = taskflow1.composed_of(taskflow2);

// approach 2: composition using make_module_task
xf::Task m1 = taskflow1.emplace(xf::make_module_task(taskflow2));
@endcode
