...
namespace xf {
    ...
    (specified by xf::PipeType) and is associated with a callable to invoke
    by the pipeline scheduler.

    You need to use the template function, xf::make_data_pipe, to create
    a data pipe. The input and output types of a xf::DataPipe should be decayed types
    ...
    @code{.cpp}
    xf::make_data_pipe<int, std::string>(
      xf::PipeType::SERIAL,
    ...
    of xf::Pipeflow in the second argument to probe the runtime information
    for a stage task, such as its line number and token number:

    @code{.cpp}
    xf::make_data_pipe<int, std::string>(
      xf::PipeType::SERIAL,
      [](int& input, xf::Pipeflow& pf) {
    ...
    */
    ...
    public:
        ...
        */
        ...
        You should use the helper function, xf::make_data_pipe,
        to create a DataPipe object, especially when you need xf::DataPipe
        to automatically deduct the lambda type.
        */
        DataPipe(PipeType d, callable_t &&callable) : _type{d}, _callable{std::forward<callable_t>(callable)} {
        }

        /**
        @brief queries the type of the data pipe

        A data pipe can be either parallel (xf::PipeType::PARALLEL) or serial
        (xf::PipeType::SERIAL).
        */
        PipeType type() const {
            return _type;
    ...
    /**
    @brief function to construct a data pipe (xf::DataPipe)

    @tparam Input input data type
    @tparam Output output data type
    @tparam C callable type

    xf::make_data_pipe is a helper function to create a data pipe (xf::DataPipe)
    in a data-parallel pipeline (xf::DataPipeline).
    The first argument specifies the direction of the data pipe,
    either xf::PipeType::SERIAL or xf::PipeType::PARALLEL,
    ...
    The callable can additionally take a reference of xf::Pipeflow,
    ...
    */
    ...
    Similar to xf::Pipeline, a xf::DataPipeline is a composable graph object
    for users to create a <i>data-parallel pipeline scheduling framework</i>
    using a module task in a taskflow.
    The only difference is that xf::DataPipeline provides a data abstraction
    ...
    int main() {
      ...
      xf::DataPipeline pl(num_lines,
        xf::make_data_pipe<void, int>(xf::PipeType::SERIAL, [&](xf::Pipeflow& pf) -> int{
          if(pf.token() == 5) {
            pf.stop();
            return 0;
          }
    ...
    */
    ...
    // constructor
    template<typename... Ps>
    DataPipeline<Ps...>::DataPipeline(size_t num_lines, Ps &&... ps) : _pipes{std::make_tuple(std::forward<Ps>(ps)...)},
    ...
    // constructor
    template<typename... Ps>
    DataPipeline<Ps...>::DataPipeline(size_t num_lines, std::tuple<Ps...> &&ps) : _pipes{
    ...
    void DataPipeline<Ps...>::_on_pipe(Pipeflow &pf, NonpreemptiveRuntime &) {
        visit_tuple([&](auto &&pipe) {
            ...
            if constexpr (std::is_invocable_v<callable_t, Pipeflow &>) {
                // [](xf::Pipeflow&) -> void {}, i.e., we only have one pipe
                if constexpr (std::is_void_v<output_t>) {
                    pipe._callable(pf);
                    // [](xf::Pipeflow&) -> output_t {}
                } else {
