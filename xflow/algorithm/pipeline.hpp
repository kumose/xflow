...
namespace xf {
    ...
    Pipeflow can only be created privately by the xf::Pipeline and
    ...
    */
    ...
    @em parallel direction or @em serial direction (specified by xf::PipeType)
    and is coupled with a callable to invoke by the pipeline scheduler.
    The callable must take a referenced xf::Pipeflow object in the first argument:

    @code{.cpp}
    Pipe{PipeType::SERIAL, [](xf::Pipeflow&){}}
    @endcode
    ...
    */
    template<typename C = std::function<void(xf::Pipeflow &)> >
    class Pipe {
        template<typename... Ps>
        friend class Pipeline;
    ...
    public:
        ...
        */
        ...
        @param d pipe type (xf::PipeType)
        @param callable callable type

        The constructor constructs a pipe with the given direction
        (xf::PipeType::SERIAL or xf::PipeType::PARALLEL) and the given callable.
        The callable must take a referenced xf::Pipeflow object in the first argument.
        ...
        (xf::PipeType::SERIAL).
        */
        Pipe(PipeType d, C &&callable) : _type{d}, _callable{std::forward<C>(callable)} {
        }
        ...
        */
        ...
        @param type a xf::PipeType variable
        */
        ...
        @param callable a callable object constructible from std::function<void(xf::Pipeflow&)>
    ...
    xf::Pipeline pipeline(num_lines,
    ...
    xf::Task task = taskflow.composed_of(pipeline)
                            .name("pipeline");
    ...
    Internally, xf::Pipeline uses std::tuple to store the given sequence of pipes.
    ...
    If applications need to change these pipes, please use xf::ScalablePipeline.
    */
    template<typename... Ps>
    class Pipeline {
    ...
    public:
        ...
        The first pipe must define a serial direction (xf::PipeType::SERIAL)
    ...
    // constructor
    template<typename... Ps>
    Pipeline<Ps...>::Pipeline(size_t num_lines, Ps &&... ps) : _pipes{std::make_tuple(std::forward<Ps>(ps)...)},
    ...
    // constructor
    template<typename... Ps>
    Pipeline<Ps...>::Pipeline(size_t num_lines, std::tuple<Ps...> &&ps) : _pipes{std::forward<std::tuple<Ps...> >(ps)},
                                                                          _meta{
                                                                              _gen_meta(
                                                                                  std::forward<std::tuple<Ps...> >(ps),
    ...
    void Pipeline<Ps...>::_build() {
        ...
        // line task
        for (size_t l = 0; l < num_lines(); l++) {
            _tasks[l + 1] = fb.emplace([this, l](xf::NonpreemptiveRuntime &rt) mutable {
    ...
    <i>pipeline scheduling framework</i> using a module task in a taskflow.
    Unlike xf::Pipeline that instantiates all pipes upon the construction time,
    xf::ScalablePipeline allows variable assignments of pipes using range iterators.
    ...
    auto pipe_callable = [&buffer] (xf::Pipeflow& pf) mutable {
      switch(pf.pipe()) {
    ...
    for(size_t i=0; i<3; i++) {
      pipes.emplace_back(xf::PipeType::SERIAL, pipe_callable);
    }

    // create a pipeline of four parallel lines based on the given vector of pipes
    xf::ScalablePipeline pl(num_lines, pipes.begin(), pipes.end());
    ...
    Each pipe has the same type of `%xf::Pipe<%std::function<void(%xf::Pipeflow&)>>`
    ...
    */
    ...
    // constructor
    template<typename P>
    ScalablePipeline<P>::ScalablePipeline(size_t num_lines) : _tasks(num_lines + 1),
                                                              _pipeflows(num_lines) {
        if (num_lines == 0) {
            TF_THROW("must have at least one line");
        }
    ...
    // constructor
    template<typename P>
    ScalablePipeline<P>::ScalablePipeline(size_t num_lines, P first, P last) : _tasks(num_lines + 1),
                                                                               _pipeflows(num_lines) {
        if (num_lines == 0) {
            TF_THROW("must have at least one line");
        }
    ...
    // move constructor
    template<typename P>
    ScalablePipeline<P>::ScalablePipeline(ScalablePipeline &&rhs) : _num_tokens{rhs._num_tokens},
