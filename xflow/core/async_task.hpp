...
namespace xf {
    ...
    A xf::AsyncTask is a lightweight handle that retains @em shared ownership
    ...
    xf::AsyncTask A = executor.silent_dependent_async([](){});
    ...
    xf::AsyncTask B = executor.silent_dependent_async([](){}, A);
    @endcode

    xf::AsyncTask is implemented based on the logic of
    ...
    */
    ...
    public:
        ...
        */
        ...
        @code{.cpp}
        xf::AsyncTask task = executor.silent_dependent_async([](){});
        ...
        */
        ...
        (with `memory_order_relaxed` load) the number of xf::AsyncTask instances that manage
        the current task.
    ...
    // Copy Constructor
    inline AsyncTask::AsyncTask(const AsyncTask &rhs) : _node{rhs._node} {
        _incref();
    }

    // Move Constructor
    inline AsyncTask::AsyncTask(AsyncTask &&rhs) : _node{rhs._node} {
        rhs._node = nullptr;
    }
