...
namespace xf {
    ...
    */
    ...
    The xf::ObserverInterface class allows users to define custom methods to monitor
    ...
    xf::ObserverInterface provides immutable wrappers,
    xf::WorkerView and xf::TaskView, over workers and tasks.

    Please refer to xf::WorkerView and xf::TaskView for details.

    Example usage:

    @code{.cpp}

    struct MyObserver : public xf::ObserverInterface {
      ...
      void on_entry(WorkerView w, xf::TaskView tv) override final {
        std::ostringstream oss;
        oss << "worker " << w.id() << " ready to run " << tv.name() << '\n';
        std::cout << oss.str();
      }

      void on_exit(WorkerView w, xf::TaskView tv) override final {
        std::ostringstream oss;
    ...
    */
    ...
    A xf::ChromeObserver inherits xf::ObserverInterface and defines methods to dump
    ...
    // create a custom observer
    std::shared_ptr<xf::ChromeObserver> observer = executor.make_observer<xf::ChromeObserver>();
    ...
    */
    ...
    A xf::TFProfObserver inherits xf::ObserverInterface and defines methods to dump
    the observed thread activities into a format that can be visualized through
    @TFProf.
    ...
    // create a custom observer
    std::shared_ptr<xf::TFProfObserver> observer = executor.make_observer<xf::TFProfObserver>();
    ...
    inline TFProfManager::~TFProfManager() {
        ...
        if (ofs) {
            ...
            // .json
                ...
                // if(_fpath.rfind(".json") != std::string::npos) {
                ofs << "[\n";
                for (size_t i = 0; i < _observers.size(); ++i) {
                    if (i) ofs << ',';
