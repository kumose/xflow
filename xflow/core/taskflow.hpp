...
#include <xflow/core/flow_builder.hpp>
...
namespace xf {
    ...
    dependency between two tasks. A task is one of the following types:
      ...
      2. subflow task        : the callable constructible from
                               @c std::function<void(xf::Subflow&)>
      3. condition task      : the callable constructible from
                               @c std::function<int()>
      4. multi-condition task: the callable constructible from
                               @c %std::function<xf::SmallVector<int>()>
      5. module task         : the task constructed from xf::Taskflow::composed_of
                               @c std::function<void(xf::Runtime&)>
    ...
    xf::Task A = taskflow.emplace([](){ std::cout << "TaskA\n"; });
    xf::Task B = taskflow.emplace([](){ std::cout << "TaskB\n"; });
    xf::Task C = taskflow.emplace([](){ std::cout << "TaskC\n"; });
    xf::Task D = taskflow.emplace([](){ std::cout << "TaskD\n"; });
    ...
    */
    ...
    public:
        ...
        @code{.cpp}
        xf::Taskflow taskflow("My Taskflow");
        ...
        */
        ...
        @code{.cpp}
        xf::Taskflow taskflow1(std::move(taskflow2));
        ...
        */
        ...
        @code{.cpp}
        xf::Task parent = taskflow.emplace([](xf::Subflow sf){
        ...
        */
        ...
        This method is similar to xf::Taskflow::dump(std::ostream& ostream),
        ...
        */
        ...
        Tasks that are created dynamically, such as those via xf::Subflow, are not counted.
        ...
        */
        ...
        An empty taskflow has no tasks, i.e., the return of xf::Taskflow::num_tasks is `0`.
        ...
        */
        ...
        A visitor is a callable that takes an argument of type xf::Task
        and returns nothing. The following example iterates each task in a
        taskflow and prints its name:

        @code{.cpp}
        taskflow.for_each_task([](xf::Task task){
          std::cout << task.name() << '\n';
        });
        @endcode
        */
        ...
        */
        ...
        A graph object is of type xf::Graph and stores a task dependency graph that can be executed
        by an xf::Executor.
    ...
    // Constructor
    inline Taskflow::Taskflow(const std::string &name) : FlowBuilder{_graph},
                                                         _name{name} {
    }
    ...
    xf::Future is a derived class from std::future that will eventually hold the
    execution result of a submitted taskflow (xf::Executor::run series).
    In addition to the base methods inherited from std::future,
    you can call xf::Future::cancel to cancel the execution of the running taskflow
    ...
    // submit the taskflow
    xf::Future fu = executor.run(taskflow);
    ...
    */
    ...
    public:
        ...
        */
        ...
        You can call xf::Future::wait to wait for the cancellation to complete.
    ...
    template<typename T>
    Future<T>::Future(std::future<T> &&f, std::weak_ptr<Topology> p) : std::future<T>{std::move(f)},
