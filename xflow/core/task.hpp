...
namespace xf {
    ...
    */
    inline const char *to_string(TaskType type) {
        ...
        switch (type) {
            case TaskType::PLACEHOLDER: val = "placeholder";
                break;
            case TaskType::STATIC: val = "static";
                break;
            case TaskType::RUNTIME: val = "runtime";
                break;
            case TaskType::SUBFLOW: val = "subflow";
                break;
            case TaskType::CONDITION: val = "condition";
                break;
            case TaskType::MODULE: val = "module";
                break;
            case TaskType::ASYNC: val = "async";
                break;
    ...
    */
    template<typename C>
    struct is_subflow_task<C, std::enable_if_t<std::is_invocable_v<C, xf::Subflow &> > >
            : std::is_same<std::invoke_result_t<C, xf::Subflow &>, void> {
    };
    ...
    */
    template<typename C>
    struct is_runtime_task<C, std::enable_if_t<std::is_invocable_v<C, xf::Runtime &> > >
            : std::is_same<std::invoke_result_t<C, xf::Runtime &>, void> {
    };
    ...
    */
    template<typename C>
    struct is_runtime_task<C, std::enable_if_t<std::is_invocable_v<C, xf::NonpreemptiveRuntime &> > >
            : std::is_same<std::invoke_result_t<C, xf::NonpreemptiveRuntime &>, void> {
    };
    ...
    */
    ...
    std::function<xf::SmallVector<int>()>.
    */
    template<typename C>
    ...
    A task created from a taskflow can be one of the following types:
      + xf::TaskType::STATIC - @ref StaticTasking
      + xf::TaskType::CONDITION - @ref ConditionalTasking
      + xf::TaskType::RUNTIME - @ref RuntimeTasking
      + xf::TaskType::SUBFLOW - @ref SubflowTasking
      + xf::TaskType::MODULE - @ref ComposableTasking

    @code{.cpp}
    xf::Task task1 = taskflow.emplace([](){}).name("static task");
    xf::Task task2 = taskflow.emplace([](){ return 3; }).name("condition task");
    xf::Task task3 = taskflow.emplace([](xf::Runtime&){}).name("runtime task");
    xf::Task task4 = taskflow.emplace([](xf::Subflow& sf){
      xf::Task stask1 = sf.emplace([](){});
      xf::Task stask2 = sf.emplace([](){});
    }).name("subflow task");
    xf::Task task5 = taskflow.composed_of(taskflow2).name("module task");
    @endcode

    A xf::Task is polymorphic.
    Once created, you can assign a different task type to it using xf::Task::work.
    For example, the code below creates a static task and then reworks it to a subflow task:

    @code{.cpp}
    xf::Task task = taskflow.emplace([](){}).name("static task");
    task.work([](xf::Subflow& sf){
      xf::Task stask1 = sf.emplace([](){});
      xf::Task stask2 = sf.emplace([](){});
    }).name("subflow task");
    @endcode
    ...
    @attention
    xf::Task does not own the lifetime of the associated node.
    ...
    */
    ...
    public:
        ...
        */
        ...
        xf::Task A = taskflow.emplace([](){ std::cout << "Task A\n"; });
        xf::Task B(A);
        ...
        */
        ...
        @code{.cpp}
        xf::Task A = taskflow.emplace([](){ std::cout << "A\n"; });
        xf::Task B;
        ...
        */
        ...
        @code{.cpp}
        xf::Task A = taskflow.emplace([](){ std::cout << "A\n"; });
        xf::Task B = taskflow.emplace([](){ std::cout << "B\n"; });
        ...
        */
        ...
        @code{.cpp}
        xf::Task A = taskflow.emplace([](){});
        xf::Task B = taskflow.emplace([](){});
        ...
        */
        ...
        @code{.cpp}
        xf::Task task = taskflow.emplace([](){}).name("foo");
        ...
        */
        ...
        @note
        To know more about xf::Semaphore, please refer to @ref LimitTheMaximumConcurrency.
        */
        ...
        */
        ...
        xf::Taskflow taskflow("attach data to a task");
        ...
        */
        ...
        A placeholder task is created from xf::Taskflow::placeholder and is associated with
        ...
        */
        ...
        @code{.cpp}
        xf::Task task = taskflow.placeholder();
        ...
        */
        ...
        @tparam V a callable type (function, lambda, etc.) that accepts a xf::Task handle
        ...
        task1.for_each_successor([](xf::Task successor){
        ...
        */
        ...
        task1.for_each_predecessor([](xf::Task predecessor){
        ...
        */
        ...
        @code{.cpp}
        xf::Task task = taskflow.emplace([](xf::Subflow& sf){
          xf::Task stask1 = sf.emplace([](){}).name("stask1");
          xf::Task stask2 = sf.emplace([](){}).name("stask2");
        });
        // Iterate tasks in the subflow and print each subflow task.
        task.for_each_subflow_task([](xf::Task stask){
        ...
        */
        ...
        A task can be one of the types defined in xf::TaskType and can be printed in
        a human-readable form using xf::to_string.

        @code{.cpp}
        auto task = taskflow.emplace([](){}).name("task");
        std::cout << task.name() << " type=[" << xf::to_string(task.type()) << "]\n";
        @endcode
        ...
        */
        ...
        @return C-styled pointer to the attached user data by xf::Task::data(void* data)
...
namespace std {
    ...
    @brief hash specialization for std::hash<xf::Task>
    */
    template<>
    struct hash<xf::Task> {
        auto operator()(const xf::Task &task) const noexcept {
    ...
    @brief hash specialization for std::hash<xf::TaskView>
    */
    template<>
    struct hash<xf::TaskView> {
        auto operator()(const xf::TaskView &task_view) const noexcept {
