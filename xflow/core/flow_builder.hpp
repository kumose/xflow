...
#include <xflow/core/task.hpp>
#include <xflow/algorithm/partitioner.hpp>
...
namespace xf {
    ...
    from which xf::Taskflow and xf::Subflow are derived.
    ...
    */
    ...
    public:
        ...
        */
        ...
        @return a xf::Task handle

        The following example creates a static task.

        @code{.cpp}
        xf::Task static_task = taskflow.emplace([](){});
        @endcode
        ...
        */
        ...
        @tparam C callable type constructible from std::function<void(xf::Runtime&)>
        ...
        @code{.cpp}
        xf::Task static_task = taskflow.emplace([](xf::Runtime&){});
        @endcode
        ...
        */
        ...
        @tparam C callable type constructible from std::function<void(xf::Subflow&)>

        @param callable callable to construct a dynamic task

        @return a xf::Task handle

        The following example creates a dynamic task (xf::Subflow)
        that spawns two static tasks.

        @code{.cpp}
        xf::Task dynamic_task = taskflow.emplace([](xf::Subflow& sf){
          xf::Task static_task1 = sf.emplace([](){});
          xf::Task static_task2 = sf.emplace([](){});
        });
        @endcode
        ...
        */
        ...
        xf::Task C = taskflow.emplace([](){ std::cout << "C"; });
        xf::Task D = taskflow.emplace([](){ std::cout << "D"; });
        A.precede(B, C, D);
        ...
        */
        ...
        xf::Task comp = t2.composed_of(t1);
        xf::Task init = t2.emplace([](){ std::cout << "t2"; });
        init.precede(comp);
        @endcode
        ...
        type xf::Graph such that it can interact with the executor.
        For example:
        ...
        struct MyObj {
          xf::Graph graph;
          MyObj() {
            xf::FlowBuilder builder(graph);
            xf::Task task = builder.emplace([](){
              std::cout << "a task\n";  // static task
            });
          }
        ...
        MyObj obj;
        xf::Task comp = taskflow.composed_of(obj);
        @endcode
        ...
        */
        ...
        xf::Task placeholder = taskflow.placeholder();
        ...
        */
        ...
        xf::Task C = taskflow.emplace([](){ std::cout << "C"; });
        xf::Task D = taskflow.emplace([](){ std::cout << "D"; });
        std::vector<xf::Task> tasks {A, B, C, D}
        ...
        */
        ...
        @tparam P partitioner type (default xf::DefaultPartitioner)
        ...
        */
        ...
        @tparam R index range type (xf::IndexRange)
        @tparam C callable type
        @tparam P partitioner type (default xf::DefaultPartitioner)
        ...
        @code{.cpp}
        // [0, 17) with a step size of 2 using xf::IndexRange
        xf::IndexRange<int> range(0, 17, 2);

        // parallelize the sequence [0, 2, 4, 6, 8, 10, 12, 14, 16]
        taskflow.for_each_by_index(range, [](xf::IndexRange<int> range) {
        ...
        The callable needs to take a single argument of type xf::IndexRange.
        ...
        */
        ...
        taskflow.reduce_by_index(
          xf::IndexRange<size_t>(0, N, 1),
          // final result
          res,
          // local reducer
          [&](xf::IndexRange<size_t> subrange, std::optional<int> running_total) -> int {
      ...
      */
        ...
        */
        ...
        @param part partitioning algorithm (default xf::DefaultPartitioner)
    ...
    Task FlowBuilder::emplace(C &&c) {
        if constexpr (std::is_invocable_v<C, xf::Runtime &>) {
            return Task(_graph._emplace_back(NSTATE::NONE, ESTATE::NONE, DefaultTaskParams{}, nullptr, nullptr, 0,
                                             std::in_place_type_t<Node::Runtime>{}, std::forward<C>(c)
            ));
        } else if constexpr (std::is_invocable_v<C, xf::NonpreemptiveRuntime &>) {
            return Task(_graph._emplace_back(NSTATE::NONE, ESTATE::NONE, DefaultTaskParams{}, nullptr, nullptr, 0,
    ...
    xf::Subflow is spawned from the execution of a task to dynamically manage a
    child graph that may depend on runtime variables.
    You can explicitly join a subflow by calling xf::Subflow::join, respectively.
    ...
    xf::Task A = taskflow.emplace([](){}).name("A");
    xf::Task C = taskflow.emplace([](){}).name("C");
    xf::Task D = taskflow.emplace([](){}).name("D");

    // create a subflow graph (dynamic tasking)
    xf::Task B = taskflow.emplace([] (xf::Subflow& subflow) {
      xf::Task B1 = subflow.emplace([](){}).name("B1");
      xf::Task B2 = subflow.emplace([](){}).name("B2");
      xf::Task B3 = subflow.emplace([](){}).name("B3");
      B1.precede(B3);
      B2.precede(B3);
    }).name("B");
    ...
    */
    ...
    // Constructor
    inline Subflow::Subflow(Executor &executor, Worker &worker, Node *parent, Graph &graph) : FlowBuilder{graph},
        _executor{executor},
        _worker{worker},
        _parent{parent} {
