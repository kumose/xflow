...
#include <xflow/core/observer.hpp>
#include <xflow/core/taskflow.hpp>
#include <xflow/core/async_task.hpp>
#include <xflow/core/freelist.hpp>
...
namespace xf {
    ...
    An xf::Executor manages a set of worker threads to run tasks
    ...
    xf::Task A = taskflow.emplace([] () { std::cout << "This is TaskA\n"; });
    xf::Task B = taskflow.emplace([] () { std::cout << "This is TaskB\n"; });
    xf::Task C = taskflow.emplace([] () { std::cout << "This is TaskC\n"; });

    // Build precedence between tasks
    A.precede(B, C);

    xf::Future<void> fu = executor.run(taskflow);
    ...
    @note
    To know more about xf::Executor, please refer to @ref ExecuteTaskflow.
    */
    class Executor {
    ...
    public:
        ...
        via deriving an instance from xf::WorkerInterface.
        ...
        */
        ...
        @param taskflow a xf::Taskflow object

        @return a xf::Future that holds the result of the execution

        This member function executes the given taskflow once and returns a xf::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        xf::Future<void> future = executor.run(taskflow);
        // do something else
        future.wait();
        @endcode
        ...
        */
        xf::Future<void> run(Taskflow &taskflow);

        /**
        @brief runs a moved taskflow once

        @param taskflow a moved xf::Taskflow object

        @return a xf::Future that holds the result of the execution

        This member function executes a moved taskflow once and returns a xf::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved taskflow.

        @code{.cpp}
        xf::Future<void> future = executor.run(std::move(taskflow));
        // do something else
        future.wait();
        @endcode
        ...
        */
        ...
        This member function returns a xf::Future object that
        eventually holds the result of the execution.

        @code{.cpp}
        xf::Future<void> future = executor.run(taskflow, [](){ std::cout << "done"; });
        // do something else
        future.wait();
        @endcode
        ...
        */
        template<typename C>
        xf::Future<void> run(Taskflow &taskflow, C &&callable);
        ...
        @code{.cpp}
        xf::Future<void> future = executor.run(
        ...
        */
        ...
        This member function executes the given taskflow @c N times and returns a xf::Future
        object that eventually holds the result of the execution.

        @code{.cpp}
        xf::Future<void> future = executor.run_n(taskflow, 2);  // run taskflow 2 times
        // do something else
        future.wait();
        @endcode
        ...
        This member function is thread-safe.
        */
        xf::Future<void> run_n(Taskflow &taskflow, size_t N);
        ...
        This member function executes a moved taskflow @c N times and returns a xf::Future
        object that eventually holds the result of the execution.
        The executor will take care of the lifetime of the moved taskflow.

        @code{.cpp}
        xf::Future<void> future = executor.run_n(
        ...
        */
        template<typename C>
        xf::Future<void> run_n(Taskflow &taskflow, size_t N, C &&callable);

        /**
        @brief runs a moved taskflow for @c N times and then invokes a callback

        @param taskflow a moved xf::Taskflow
        @param N number of runs
        @param callable a callable object to be invoked after this run
        ...
        */
        ...
        @code{.cpp}
        xf::Future<void> future = executor.run_until(
        ...
        */
        template<typename P>
        xf::Future<void> run_until(Taskflow &taskflow, P &&pred);
        ...
        */
        ...
        xf::Future<void> run_until(Taskflow &taskflow, P &&pred, C &&callable);
        ...
        */
        ...
        @tparam T target type which has `xf::Graph& T::graph()` defined
        ...
        xf::Taskflow taskflow;
        std::array<xf::Taskflow, 1000> others;
        ...
        @attention
        You must call xf::Executor::corun from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename T>
        void corun(T &target);
        ...
        @attention
        You must call xf::Executor::corun_until from a worker of the calling executor
        or an exception will be thrown.
        */
        template<typename P>
        ...
        */
        ...
        @code{.cpp}
        xf::Executor executor(4);   // 4 workers in the executor
        ...
        */
        ...
        @tparam Observer observer type derived from xf::ObserverInterface
        ...
        xf::ObserverInterface::on_entry and xf::ObserverInterface::on_exit
        ...
        */
        ...
        /**
        @brief similar to xf::Executor::async but does not return a future object
        ...
        This member function is more efficient than xf::Executor::async
        ...
        */
        ...
        @tparam F callable type
        @tparam Tasks task types convertible to xf::AsyncTask

        @param func callable object
        @param tasks asynchronous tasks on which this execution depends

        @return a xf::AsyncTask handle

        This member function is more efficient than xf::Executor::dependent_async
        ...
        @code{.cpp}
        xf::AsyncTask A = executor.silent_dependent_async([](){ printf("A\n"); });
        xf::AsyncTask B = executor.silent_dependent_async([](){ printf("B\n"); });
        ...
        */
        ...
        >
        xf::AsyncTask silent_dependent_async(F &&func, Tasks &&... tasks);
        ...
        @code{.cpp}
        xf::AsyncTask A = executor.silent_dependent_async("A", [](){ printf("A\n"); });
        xf::AsyncTask B = executor.silent_dependent_async("B", [](){ printf("B\n"); });
        ...
        */
        ...
        >
        xf::AsyncTask silent_dependent_async(P &&params, F &&func, Tasks &&... tasks);
        ...
        @code{.cpp}
        std::array<xf::AsyncTask, 2> array {
        ...
        */
        ...
        >
        xf::AsyncTask silent_dependent_async(F &&func, I first, I last);
        ...
        */
        ...
        >
        xf::AsyncTask silent_dependent_async(P &&params, F &&func, I first, I last);
        ...
        @return a pair of a xf::AsyncTask handle and
                          a @std_future that holds the result of the execution

        The example below creates three asynchronous tasks, @c A, @c B, and @c C,
        in which task @c C runs after task @c A and task @c B.
        Task @c C returns a pair of its xf::AsyncTask handle and a std::future<int>
        that eventually will hold the result of the execution.
        ...
        You can mix the use of xf::AsyncTask handles
        returned by xf::Executor::dependent_async and xf::Executor::silent_dependent_async
    ...
    private:
        ...
        void _process_dependent_async(Node *, xf::AsyncTask &, size_t &);
    ...
    // Constructor
    inline Executor::Executor(size_t N, std::unique_ptr<WorkerInterface> wix) : _workers(N),
        _notifier(N),
        _buffers(N),
...
#define TF_INVOKE_CONTINUATION()  \
  if (cache) {                      \
    ...
    // Function: run
    inline xf::Future<void> Executor::run(Taskflow &f) {
        return run_n(f, 1, []() {
        });
    }
    ...
    // Function: run
    template<typename C>
    xf::Future<void> Executor::run(Taskflow &f, C &&c) {
    ...
    // Function: run_n
    inline xf::Future<void> Executor::run_n(Taskflow &f, size_t repeat) {
    ...
    // Function: run_n
    template<typename C>
    xf::Future<void> Executor::run_n(Taskflow &f, size_t repeat, C &&c) {
        return run_until(
    ...
    // Function: run_until
    template<typename P>
    xf::Future<void> Executor::run_until(Taskflow &f, P &&pred) {
    ...
    // Function: run_until
    template<typename P, typename C>
    xf::Future<void> Executor::run_until(Taskflow &f, P &&p, C &&c) {
        _increment_topology();
        ...
        if (f.empty() || p()) {
            ...
            promise.set_value();
            _decrement_topology();
            return xf::Future<void>(promise.get_future());
        }
        ...
        xf::Future<void> future(t->_promise.get_future(), t);
    ...
    // Function: run_until
    template<typename P, typename C>
    xf::Future<void> Executor::run_until(Taskflow &&f, P &&pred, C &&c) {
