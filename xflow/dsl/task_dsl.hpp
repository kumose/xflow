...
#include <xflow/dsl/meta_macro.hpp>
#include <xflow/dsl/task_analyzer.hpp>
#include <xflow/dsl/task_trait.hpp>
...
#define TF_CONTEXT_1(name) xf::dsl::EmptyContext
...
#define make_task(name, ...)                                                    \
  struct TF_GET_FIRST name : xf::dsl::TaskSignature,                           \
...
#define some_tasks(...) auto (*)(xf::dsl::SomeTask<__VA_ARGS__>)
// same as some_tasks
...
#define build_taskflow(...) xf::dsl::taskDsl<void TF_MAP(TF_CHAIN, __VA_ARGS__)>
