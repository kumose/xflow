...
size_t skynet_one_tf(xf::Runtime& rt, size_t BaseNum, size_t Depth, size_t MaxDepth) {
  ...
  for (size_t i = 0; i < 10; ++i) {
    rt.silent_async([=, &results](xf::Runtime& s) {
...
void skynet(size_t num_threads, size_t MaxDepth) {
  ...
  executor.async([=](xf::Runtime& rt) { skynet_one_tf(rt, 0, 0, MaxDepth); }).wait();
}
